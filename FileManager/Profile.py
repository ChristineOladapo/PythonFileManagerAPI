...
# curious since we already covered a bit of the JSON format in class.
...
your own code. It is raised when attempting to load or
save Profile objects to file the system.
...
attempting to deserialize a dsu file to a Profile object.
...
class Post(dict):
    ...
    user posts. It currently supports two features: A timestamp
    property that is set upon instantiation and when the entry
    object is set and an entry property that stores the post message.
    ...
    time values. When the value for entry is changed, or set, the
...
class Profile:
    """
    The Profile class exposes the properties required to join an ICS
    32 DSU server. You will need to use this class to manage the
    information provided by each new user created within your program
    for a2. Pay close attention to the properties and functions
    in this class as you will need to make use of each of them in your program.

    When creating your program you will need to collect user input
    for the properties exposed by this class. A Profile class should
    ensure that a username and password are set, but contains no
